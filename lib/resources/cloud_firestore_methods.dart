...
class CloudFireStore{
  ...
  Future uploadNameandAddressToDatabase({required UserDetailsModel user})async{
  ...
  Future getNameAndAddress()async{
    ...
    UserDetailsModel userDetailsModel=UserDetailsModel.getModelFromJson(snap.data() as dynamic);
  ...
  Future<String> upLoadProductToDatabase({
    ...
    if(image != null && productName != "" && rawCost != ""){
      try{
        ...
        ProductModel product=ProductModel(url: url,productName: productName,cost: cost,discount: discount,uid:uid,sellerName: sellerName,sellerUid: sellerUid,rating:5 ,noOfRating:0 );
  ...
  Future<String> upLoadImage({required Uint8List image,required String uid})async{
  ...
  Future<List<Widget>>getProductDiscount(int discount)async{
    ...
    for(int i=0;i<snap.docs.length;i++){
      DocumentSnapshot documentSnap= snap.docs[i];
      ProductModel model=ProductModel.getModelFromJson(json: documentSnap.data() as dynamic);
  ...
  Future addProductToOrders({required ProductModel model,required UserDetailsModel userDetails})async{
    await firebaseFirestore.collection("users")
        .doc(firebaseAuth.currentUser!.uid)
        .collection("orders").add(model.getJson());
  ...
  Future sendOrderRequest({required ProductModel model,required UserDetailsModel userDetails})async{
    OrderRequesModel orderRequesModel=OrderRequesModel(orderName: model.productName, buyerAddress: userDetails.address);
    await firebaseFirestore.collection('users').doc(model.sellerUid).collection('orderRequests').add(orderRequesModel.getJson());
  }
